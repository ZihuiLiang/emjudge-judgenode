...
async fn main() -> Result<(), Error> {
    ...
    let url = format!("http://{}:{}/info/test_pressure", settings.url, settings.port);
    ...
    loop {
        ...
        let result: emjudge_judgenode::responses::TestPressureResponse = serde_json::from_str(&response.text().await.unwrap()).unwrap();
        println!("{:?}", result);
