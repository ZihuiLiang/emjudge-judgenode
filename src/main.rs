...
use emjudge_judgenode::{
    service::{
        auth::{is_authed, Authentication},
        info, test_data, tests,
    },
    settings::Settings,
};
use env_logger::Env;
...
async fn main() -> std::io::Result<()> {
    ...
    HttpServer::new(move || {
        App::new()
            ...
            .service(
                web::scope("/test")
                    .service(tests::only_run)
                    .service(tests::run_and_eval)
                    .service(tests::ans_and_eval)
                    .service(tests::run_and_interact),
            )
            ...
            .service(
                web::scope("/info")
                    ...
                    .service(info::mem)
                    .service(info::sys)
                    .service(info::language)
                    .service(info::languages)
                    .service(info::test_pressure),
            )
    })
    .bind((url, port))?
    .workers(workers)
