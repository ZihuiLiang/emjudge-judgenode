...
use emjudge_judgenode::{
    data::TestData, requests::TestRequest, responses::OnlyRunResponse, settings::Settings,
};
use reqwest::{
...
async fn hello_world() -> Result<(), Error> {
    ...
    std::fs::File::open("tests/programs/helloworld/main.cpp")
        .unwrap()
    ...
    let response = reqwest::Client::new()
        ...
        .unwrap();
    assert_eq!(response.status().as_u16(), 200);
    let result: Vec<OnlyRunResponse> =
...
async fn mle() -> Result<(), Error> {
    ...
    let mut tested_code = vec![];
    std::fs::File::open("tests/programs/mle.cpp")
        .unwrap()
        .read_to_end(&mut tested_code)
        .unwrap();
    let test_data = TestData {
