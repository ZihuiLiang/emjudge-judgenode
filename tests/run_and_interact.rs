...
use emjudge_judgenode::{
    data::TestData, requests::TestRequest, responses::RunAndInteractResponse, settings::Settings,
};
use reqwest::{
...
async fn guessnumber() -> Result<(), Error> {
    ...
    std::fs::File::open("tests/programs/guessnumber/tested.cpp")
        .unwrap()
        .read_to_end(&mut tested_code)
        .unwrap();
    std::fs::File::open("tests/programs/guessnumber/interactor.cpp")
        .unwrap()
        .read_to_end(&mut interact_code)
        .unwrap();
    ...
    let response = reqwest::Client::new()
        ...
        .unwrap();
    assert_eq!(response.status().as_u16(), 200);
    let result: Vec<RunAndInteractResponse> =
